 ...
 * POI兴趣点
 */
app.controller('RoadbookListController',[
  '$scope',
  '$http',
  '$state',
  '$stateParams',
  ...
  function($scope,$http,$state,$stateParams,dialogService,notifyService,$timeout){
    ...
    $scope.pageChanged = function(){
      ...
      $http.post('roadbook/getList',data).success(function(response){
    	  layer.close(loading);
    ...
    $scope.loadState = function(){
      $http.post('roadbook/getState').success(function(data){ 
    ...
    $scope.updateState=function(id,state,index){
    	$http.post('roadbook/updateState?id='+id+'&state='+state).success(function(data){
